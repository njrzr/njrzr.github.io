...
<template>
  <div class="relative w-full h-full bg-[url('/images/background-cv-blurred.jpg')] bg-no-repeat bg-cover bg-center">
    ...
    <div v-else class="relative flex justify-end items-start w-full h-full">
      <div v-if="data" class="relative w-1/2 h-full p-8 flex flex-col gap-4 bg-white/75 font-montserrat overflow-y-scroll scrollbar-none">
        ...
        <p class="relative text-xl text-justify">{{ data.excerpt }}</p>

        <p class="relative text-2xl border-b-2 pb-4">Experiencia Laboral</p>

        <div class="relative flex flex-col px-4" v-for="experience of data.laboral_experience">
          <div class="relative flex justify-between items-center">
            <div class="relative w-4/6 h-auto border-b-2 pb-2">
              <p class="font-semibold">{{ experience.work }}</p>
              <p>{{ experience.title }}</p>
            </div>

            <div class="relative w-2/6 h-auto border-b-2 border-l-2 pb-2 pl-6">
              <p class="font-semibold">{{ experience.country }}</p>
              <p>{{ experience.time }}</p>
            </div>
          </div>

          <p class="mt-2">{{ experience.about }}</p>
        </div>

        <p class="relative text-2xl border-b-2 pb-4">Educación</p>

        <div class="relative flex flex-col px-4" v-for="education of data.education">
          <div class="relative flex justify-between items-center">
            <div class="relative w-4/6 h-auto border-b-2 pb-2">
              <p class="font-semibold">{{ education.school }}</p>
              <p>{{ education.carrer }}</p>
            </div>

            <div class="relative w-2/6 h-auto border-b-2 border-l-2 pb-2 pl-6">
              <p class="font-semibold">{{ education.country }}</p>
              <p>{{ education.time }}</p>
            </div>
          </div>
        </div>
    ...
    <p class="absolute left-4 bottom-4 text-5xl sm:text-8xl text-white shadow-text font-montserrat">Curriculum</p>
  </div>
</template>
